// Generated by Bisonc++ V5.02.00 on Wed, 07 Nov 2018 11:56:58 +0100
...
PI__ const s_productionInfo[] = 
{
     ...
     {302, 2}, // 14: assignment (T_SEMICOLON) ->  multipleassignment T_SEMICOLON
     {307, 5}, // 15: multipleassignment (T_ID) ->  T_ID T_ASSIGNSEP multipleassignment T_ASSIGNSEP expression
     {307, 3}, // 16: multipleassignment (T_ID) ->  T_ID T_ASSIGN expression
     {303, 5}, // 17: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {304, 5}, // 18: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {305, 5}, // 19: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {305, 7}, // 20: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {306, 5}, // 21: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {306, 5}, // 22: loop (T_REPEAT) ->  T_REPEAT statements T_UNTIL expression T_DONE
     {306, 9}, // 23: loop (T_FOR) ->  T_FOR T_ID T_FROM expression T_TO expression T_DO statements T_DONE
     {308, 1}, // 24: expression (T_NUM) ->  T_NUM
     {308, 1}, // 25: expression (T_TRUE) ->  T_TRUE
     {308, 1}, // 26: expression (T_FALSE) ->  T_FALSE
     {308, 1}, // 27: expression (T_ID) ->  T_ID
     {308, 3}, // 28: expression (T_ADD) ->  expression T_ADD expression
     {308, 3}, // 29: expression (T_SUB) ->  expression T_SUB expression
     {308, 3}, // 30: expression (T_MUL) ->  expression T_MUL expression
     {308, 3}, // 31: expression (T_DIV) ->  expression T_DIV expression
     {308, 3}, // 32: expression (T_MOD) ->  expression T_MOD expression
     {308, 3}, // 33: expression (T_LESS) ->  expression T_LESS expression
     {308, 3}, // 34: expression (T_GR) ->  expression T_GR expression
     {308, 3}, // 35: expression (T_EQ) ->  expression T_EQ expression
     {308, 3}, // 36: expression (T_AND) ->  expression T_AND expression
     {308, 3}, // 37: expression (T_OR) ->  expression T_OR expression
     {308, 2}, // 38: expression (T_NOT) ->  T_NOT expression
     {308, 3}, // 39: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {309, 1}, // 40: program_$ ->  program
};
